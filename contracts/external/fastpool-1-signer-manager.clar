...
(define-public (claim-rewards
  ...
  (let ((result (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 claim-rewards
...
(define-public (claim-staker-rewards
  ...
  (let (
      ...
      (rewards-info (unwrap-panic (contract-call? 'SP000000000000000000002Q6VF78.pox-5
        claim-staker-rewards-for-signer staker reward-cycle bond-index
      )))
      (prev-fees (var-get earned-fees))
      (gross (get earned rewards-info))
      (fees (/ (* gross (get-fee-bips-for-cycle reward-cycle bond-index)) MAX_BIPS))
      (earned (- gross fees))
