...
call('B: convert half through real smart router (one full 50k chunk)',VAULT,'router-swap',[update],v=>ok(v)&&v.includes('(amount u50000)')&&v.includes('(unsold u0)'),ALICE);
recover(b);
const c=claim('C normal batch after recovery',2);
pay(b,c);
age('C liquidation phase',288);
advance();
call('C: first normal chunk through real router (50k of 100k)',VAULT,'router-swap',[update],v=>ok(v)&&v.includes('(amount u50000)')&&v.includes('(unsold u0)'),ALICE);
advance();
call('C: second normal chunk through real router (remaining 50k)',VAULT,'router-swap',[update],v=>ok(v)&&v.includes('(amount u50000)')&&v.includes('(unsold u0)'),BOB);
