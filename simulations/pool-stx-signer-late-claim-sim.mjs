// stxer sim: FORGOTTEN TRANCHE. Claims cycle 141 tranche 0, deliberately skips
// tranche 1, lets ~2 reward cycles pass, then claims and pays it late.
...
// 9. SKIP tranche 1 entirely. Fund rewards for cycle 141 and let pox-5 credit
// them at the next distribution boundary -- but DO NOT claim.
call("WHALE sends 1 sBTC again (cycle 141's 2nd credit)", "transfer",
  [uintCV(REWARD_SATS), principalCV(WHALE), principalCV(POX5_ID), noneCV()], WHALE, SBTC_ID);
steps.push("ADVANCE 1050 -> dist boundary 964250, still crediting cycle 141");
sim.addAdvanceBlocks({ bitcoin_blocks: 1050, stacks_blocks_per_bitcoin: 1 });
call("pox-5 calculate-rewards (credits cycle 141 again)", "calculate-rewards",
  [listCV([])], DEPLOYER, POX5_ID);
evalCode("unclaimed for our signer, cycle 141 -> should be NON-ZERO",
  `(get-unclaimed-signer-rewards u${CYCLE} none)`);
evalCode("tranche-count still u1 (we deliberately did NOT claim)",
  `(get-tranche-count u${CYCLE})`);

// 10. Let SEVERAL CYCLES pass with the money unclaimed.
steps.push("ADVANCE 4200 blocks (~2 whole reward cycles) -- we 'forgot'");
sim.addAdvanceBlocks({ bitcoin_blocks: 4200, stacks_blocks_per_bitcoin: 1 });
evalCode("pox-5 reward cycle now (should be ~143/144)",
  `(current-pox-reward-cycle)`, POX5_ID);
evalCode("cycle 141 rewards STILL unclaimed (no expiry in pox-5)",
  `(get-unclaimed-signer-rewards u${CYCLE} none)`);
evalCode("last-claim-dist-cycle for 141 (bookmark from tranche 0)",
  `(get-last-claim-dist-cycle u${CYCLE})`);

// 11. LATE CLAIM for the old cycle. The gate must allow it: the bookmark is
// far in the past, so dist > last-dist holds by a wide margin.
call("LATE pox-claim-rewards for cycle 141 -> expect TRANCHE 1",
  "pox-claim-rewards", [listCV([]), uintCV(CYCLE)]);
evalCode("get-tranche-count -> expect u2", `(get-tranche-count u${CYCLE})`);
evalCode("tranche 1 pot (the money we forgot)", `(get-stx-pot u${CYCLE} u1)`);

// 12. Pay the OLD cycle's stakers, long after they could have unstaked.
call("pay-stx-stakers cycle 141 TRANCHE 1 (all 8), cycles later",
  "pay-stx-stakers", [listCV(STAKERS.map((s) => principalCV(s.a))), uintCV(CYCLE), uintCV(1)]);
evalCode("tranche 1 fully paid? -> expect TRUE", `(is-tranche-fully-paid u${CYCLE} u1)`);
evalCode("OG payout, late tranche", `(get-stx-paid u${CYCLE} u1 '${STAKERS[0].a})`);
evalCode("non-OG payout, late tranche", `(get-stx-paid u${CYCLE} u1 '${STAKERS[1].a})`);
evalCode("tranche 0 unaffected", `(is-tranche-fully-paid u${CYCLE} u0)`);
